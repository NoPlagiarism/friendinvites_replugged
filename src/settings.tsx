...
export const cfg = await settings.init<Settings, keyof typeof defaultSettings>("xyz.noplagi.friendinvites", defaultSettings);
...
export function SettingsWindow(): React.ReactElement {
  return (
    <Flex style={{ gap: "1vh" }} direction={Flex.Direction.VERTICAL}>
      <SelectItem
        {...util.useSetting(cfg, "mode")}
        options={ModeOptions}>
        Message mode
      </SelectItem>
      <CheckboxItem
      {...util.useSetting(cfg, "alwaysSendSimple")}>
